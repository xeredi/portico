...
function ConfGridController($http, pageTitleService) {
	...
	$http.post("administracion/configuracion/configuration-list.action")
...
function M18nGridController($http, pageTitleService) {
	...
	$http.post("administracion/messagei18n/message-i18n-list.action").success(
			function(data) {
...
function SuperpuertoGridController($http, $routeParams, $location,
		pageTitleService) {
	var vm = this;
	...
	vm.sprtCriterio = $routeParams.sprtCriterio ? angular
			.fromJson($routeParams.sprtCriterio) : {};
				...
				function(data) {
					vm.accion == 'edit' ? setTimeout(function() {
						window.history.back();
							...
							"/administracion/superpuerto-detail/"
...
function PuertoGridController($http, $routeParams, $location, pageTitleService) {
	...
	vm.prtoCriterio = $routeParams.prtoCriterio ? angular
			.fromJson($routeParams.prtoCriterio) : {};
