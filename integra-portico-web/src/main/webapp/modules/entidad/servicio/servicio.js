...
function SrscGridController($http, $location, $routeParams, $modal,
		pageTitleService) {
	var vm = this;
	...
	function initialize() {
		vm.srscCriterio = $routeParams.srscCriterio ? angular
				.fromJson($routeParams.srscCriterio) : {};
				...
				function(data) {
					vm.accion == 'edit' ? setTimeout(function() {
						window.history.back();
							...
							"/servicio/srsc/detail/" + data.model.prto.id + "/"
									+ data.model.tpsr.id + "/"
				...
				function(data) {
					vm.accion == 'edit' ? setTimeout(function() {
						window.history.back();
							...
							"/servicio/srvc/detail/" + data.model.entiId + "/"
...
function SrvcTramiteDetailController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function SrvcTramiteEditController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function ManiTotalesController($http, $routeParams, pageTitleService) {
	...
	function initialize() {
		...
		pageTitleService.setTitleEnti($routeParams.entiId,
...
function SsrvDetailController($http, $location, $routeParams, pageTitleService) {
	...
	function initialize() {
		vm.path = $location.path();
		vm.pageMap = $routeParams.pageMap ? angular
				.fromJson($routeParams.pageMap) : {};
						...
						function(data) {
							vm.accion == 'edit' ? setTimeout(function() {
								window.history.back();
									...
									"/servicio/ssrv/detail/" + data.model.id)
									.replace();
						});
	}
...
function SsrvTramiteDetailController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function SsrvTramiteEditController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
