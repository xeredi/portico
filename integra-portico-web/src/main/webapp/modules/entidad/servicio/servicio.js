...
function config($routeProvider) {
	$routeProvider
			...
			.when(
					"/servicio/srvc/maniTotales/:entiId/:srvcId",
					{
			...
			.when(
					"/servicio/ssrv/mablTotales/:entiId/:srvcId/:ssrvId",
					{
						templateUrl : "modules/entidad/servicio/manifiesto/mani-totales.html",
			...
			.when(
					"/servicio/ssrv/atraDenegar/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraAutorizar/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraAnular/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraIniciar/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraFinalizar/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraCambiarMuelle/:srvcId/:ssrvId",
					{
			...
			.when(
					"/servicio/ssrv/atraAutorizarFPrevio/:srvcId/:ssrvId",
					{
...
function SrvcDetailController($http, $location, $routeParams, pageTitleService) {
	...
	function srvcAction(accName) {
		...
		case "mani-anular":
			...
			break;
		case "mani-totales":
			$location.path("/servicio/srvc/maniTotales/" + vm.item.entiId + "/"
					+ vm.item.id);
		...
		item : {
			...
			function(data) {
				...
				for (i = 0; i < vm.enti.entiHijasList.length; i++) {
					...
					findSublist(subentiId,
							pageMap[subentiId] ? pageMap[subentiId] : 1);
				}
			});
	...
	function save() {
		$http.post("servicio/srvc-save.action", {
			...
			accion : vm.accion
				...
				function(data) {
					$location.path(
							"/servicio/srvc/detail/" + data.item.entiId + "/"
									+ data.item.id).replace();
				});
	}
...
function SrvcDuplicateController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function SsrvGridController($http, $location, $routeParams, $modal,
		pageTitleService) {
	var vm = this;
...
function SsrvDetailController($http, $location, $routeParams, pageTitleService) {
	...
	function ssrvAction(accName) {
		...
		case "mabl-anular":
			...
			break;
		case "mabl-totales":
			$location.path("/servicio/ssrv/mablTotales/" + vm.item.entiId + "/"
					+ vm.item.srvc.id + "/" + vm.item.id);
		...
		case "atra-denegar":
			$location.path("/servicio/ssrv/atraDenegar/" + vm.item.srvc.id
					+ "/" + vm.item.id);

			break;
		case "atra-autorizar":
			$location.path("/servicio/ssrv/atraAutorizar/" + vm.item.srvc.id
					+ "/" + vm.item.id);

			break;
		case "atra-anular":
			$location.path("/servicio/ssrv/atraAnular/" + vm.item.srvc.id + "/"
					+ vm.item.id);

			break;
		case "atra-iniciar":
			$location.path("/servicio/ssrv/atraIniciar/" + vm.item.srvc.id
					+ "/" + vm.item.id);

			break;
		case "atra-finalizar":
			$location.path("/servicio/ssrv/atraFinalizar/" + vm.item.srvc.id
					+ "/" + vm.item.id);

			break;
		case "atra-cambiar-muelle":
			$location.path("/servicio/ssrv/atraCambiarMuelle/"
					+ vm.item.srvc.id + "/" + vm.item.id);

			break;
		case "atra-autorizar-fprevio":
			$location.path("/servicio/ssrv/atraAutorizarFPrevio/"
					+ vm.item.srvc.id + "/" + vm.item.id);
...
function SsrvDuplicateController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
	...
	function save() {
		$http.post("servicio/ssrv-save.action", {
			...
			accion : vm.accion
				...
				function(data) {
					$location.path(
							"/servicio/ssrv/detail/" + data.item.entiId + "/"
									+ data.item.id).replace();
				});
	}
...
function AtraAutorizarController($http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function atraCambiarMuelleController($scope, $http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
...
function atraAutorizarFPrevioController($scope, $http, $location, $routeParams,
		pageTitleService) {
	var vm = this;
