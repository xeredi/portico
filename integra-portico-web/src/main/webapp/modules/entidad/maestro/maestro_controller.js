...
function config($stateProvider) {
    ...
    .state("parametro-edit", {
        url : "/maestro/parametro/edit/:accion/:entiId?id&fref",
    ...
    .state("subparametro-edit", {
        url : "/maestro/subparametro/edit/:accion/:entiId/:prmtId?id&fref",
...
function ParametroGridController($state, $stateParams, $modal, pageTitleService, ParametroService) {
    var vm = this;
    ...
    function xlsExport() {
        ParametroService.xlsExport(vm.searchCriteria).then(function(data) {
            var file = new Blob([ data ], {
                type : 'application/xls'
            });

            setTimeout(function() {
                saveAs(file, 'prmt_' + vm.searchCriteria.entiId + '.xls');
            }, 0);
        });
    }
...
function ParametroDetailController($stateParams, pageTitleService, ParametroService, SubparametroService) {
    var vm = this;
    ...
    ParametroService.detail(vm.item).then(function(data) {
        vm.item = data.model;
...
function ParametroEditController($state, $stateParams, pageTitleService, ParametroService) {
    var vm = this;
    ...
    function save() {
        ParametroService.saveI18n(vm.accion, vm.item, vm.i18nMap).then(function(data) {
...
function SubparametroDetailController($stateParams, pageTitleService, SubparametroService) {
    var vm = this;
...
function SubparametroEditController($state, $stateParams, pageTitleService, SubparametroService) {
    var vm = this;
