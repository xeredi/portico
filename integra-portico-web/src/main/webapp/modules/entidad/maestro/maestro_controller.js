(function() {
	...
	function maestro_config($routeProvider) {
		$routeProvider
				...
				.when(
						"/maestro/parametro/detail/:entiId/:id/:fref?",
						{
				...
				.when(
						"/maestro/parametro/edit/:accion/:entiId/:id?/:fref?",
						{
				...
				.when(
						"/maestro/subparametro/detail/:entiId/:id/:fref?",
						{
				...
				.when(
						"/maestro/subparametro/edit/:accion/:entiId/:prmtId/:id?/:fref?",
						{
	...
	/* @ngInject */
	function MaestroIndexController($translate, $filter, pageTitleService,
			credentialService, MaestroService) {
		var vm = this;

		MaestroService.index().then(
				function(data) {
					vm.tpprList = $filter('filter')(
							data.resultList,
							function(element) {
								return credentialService.hasAcenPath(
										element.value, "item-list");
							});

					vm.tpprList = vm.tpprList.map(function(tppr) {
						$translate('enti_' + tppr.value).then(
