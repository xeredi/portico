...
public class I18nJs {
	...
	public void export() throws IOException {
		final String[] languages = ConfigurationProxy
				.getStringArray(ConfigurationKey.language_available);
		final String defaultLanguage = ConfigurationProxy
				.getString(ConfigurationKey.language_default);
		final String webappInstallPath = ConfigurationProxy
				.getString(ConfigurationKey.webapp_install_path);
		...
		for (final String language : languages) {
			...
			labelValues.addAll(i18nBO.selectLabelValueList(i18nPrefixSet,
					language));

			final Map<MessageI18nKey, String> messageI18nMap = messageI18nBO
					.selectKeyValueMap(new Locale(language), true);

			for (final MessageI18nKey key : messageI18nMap.keySet()) {
				labelValues.add(new LabelValueVO(key.name(), messageI18nMap
						.get(key)));
			}

			final Iterator<LabelValueVO> labelValueIterator = labelValues
					.iterator();
			...
			while (labelValueIterator.hasNext()) {
				...
				i18nParams.append(labelValue.getLabel() + ":'"
						+ labelValue.getValue() + '\'');
			...
			final String filename = webappInstallPath
					+ "/modules/i18n/i18n_messages_" + language + ".js";
