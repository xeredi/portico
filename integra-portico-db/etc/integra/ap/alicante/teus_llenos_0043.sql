--------------------------------------------------------
--  DDL for View TEUS_LLENOS_0043
--------------------------------------------------------

CREATE OR REPLACE VIEW INTEGRA.TEUS_LLENOS_0043 (
...
FROM
	...
	JOIN iman_unidadcargatemp_unic unicTmp ON unicTmp.unic_id = equi_unidad_carga_id
		...
		AND (
			unicTmp.unic_fecha_fin IS NULL
			OR unicTmp.unic_fecha_fin > equi_fecha_modificacion
		)
WHERE MABL_ES_ACTIVO = 1
	...
	AND equi.equi_lleno_vacio IN ( '7','8' )
	AND unic.unic_codigo LIKE '3%'
	AND maco_consignatario_id = '14'
	AND Equi.equi_estado LIKE 'R'
GROUP BY MANI.MANI_ID, SMAN.SERV_SUBP_ID, SUBPSMAN.SUBP_CODIGO, SMAN.SERV_ANNO, SMAN.SERV_NUMERO
...
CREATE OR REPLACE VIEW TEUS_LLENOS_0043_5 (
