--------------------------------------------------------
--  DDL for View TEUS_VACIOS_0015
--------------------------------------------------------

CREATE OR REPLACE VIEW TEUS_VACIOS_0015 (
	SMAN_ID, SMAN_SUBP_ID, SMAN_SUBP_CODIGO, SMAN_ANNO, SMAN_NUMERO, SMAN_FECHA, PAGADOR_SUBP_ID, PAGADOR_SUBP_CODIGO
	, PAGADOR_ID, PAGADOR_COD, PAGADOR_NOMBRE, MABL_ID, MABL_TIPO, MABL_ORDEN, EQUITEUS, EQUIUNIDADES
) AS
SELECT
	...
	AND (
		...
		OR EXISTS (
			...
			FROM
				...
				JOIN iesc_serviciotipo_sett ON sett_id = stst_sett_id
			WHERE
				stst_traf_id = mani_servicio_id
				AND STST_FECHA_INICIO_ADHESION <= MANI_FECHA_REFERENCIA
				AND (
					STST_FECHA_FIN_ADHESION IS NULL
					OR STST_FECHA_FIN_ADHESION > MANI_FECHA_REFERENCIA
				)
		)
	)
	AND EXISTS (
		...
		WHERE
			TTSE_SERV_ID = SERV_ID
			AND TTSE_SUBSERVICIO = EQUI_ID
			AND TTSE_ESTADO IN ( 'V', 'T' )
	)
	AND equi.equi_lleno_vacio = '4'
	AND unic.unic_codigo LIKE '3%'
	AND maco_consignatario_id = '10'
	AND Equi.equi_estado LIKE 'R'
GROUP
	BY MANI.MANI_ID, SMAN.SERV_SUBP_ID, SUBPSMAN.SUBP_CODIGO, SMAN.SERV_ANNO, SMAN.SERV_NUMERO
	, MANI.MANI_FECHA_REFERENCIA , MACO_CONSIGNATARIO_SUBP_ID , SUBPMACO.SUBP_CODIGO , MACO_CONSIGNATARIO_ID
	, ORGAMACO.ORGA_COD , ORGAMACO.ORGA_NOMBRE , mabl.mabl_id, mabl.mabl_tipo, mabl.mabl_orden
;


SELECT * FROM TEUS_VACIOS_0015;
SELECT * FROM TEUS_VACIOS_0015_5;

SELECT mabl_id, COUNT(1) AS cnt
FROM iman_bl_mabl
    INNER JOIN iman_consignatario_maco ON
        maco_id = mabl_consignatario_id
    INNER JOIN iman_equipamiento_equi ON
        equi_mabl_id = mabl_id
WHERE
    maco_consignatario_id = (
        SELECT orga_id FROM icom_organizacion_orga WHERE orga_cod = '0015'
    )
    AND EXISTS (
        SELECT 1 
        FROM ifac_tasatarifaservicio_ttse
    ...
    AND equi_lleno_vacio = '4' 
    AND equi_estado LIKE 'R'
    AND EXISTS (
        SELECT 1 FROM iman_unidadcarga_unic
...
CREATE INDEX id_tasatarifaservicio_serv ON ifac_tasatarifaservicio_ttse(ttse_subservicio, ttse_ente_id);

SELECT *
FROM (
    SELECT mabl_id, serv_id, serv_subp_id, serv_numero, serv_anno, mani_fecha_referencia, mabl_tipo
        , mabl_orden
    FROM 
        iman_bl_mabl
...
CREATE OR REPLACE VIEW TEUS_VACIOS_0015_5 (
...
SELECT 
    ...
    , (SELECT MACO_CONSIGNATARIO_SUBP_ID FROM iman_consignatario_maco WHERE maco_id = MABL_CONSIGNATARIO_ID) AS pagador_SUBP_ID
