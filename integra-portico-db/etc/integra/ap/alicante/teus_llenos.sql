--------------------------------------------------------
--  DDL for View TEUS_LLENOS
--------------------------------------------------------

CREATE OR REPLACE VIEW INTEGRA.TEUS_LLENOS (
...
SELECT
	...
	, COALESCE(
		SUM(
			UNIC_TEUS * (
				CASE when EQUI_NUMERO_VACIOS is null or EQUI_NUMERO_VACIOS = 0
					then 1
...
FROM
	...
	JOIN iman_unidadcargatemp_unic unicTmp ON unicTmp.unic_id = equi_unidad_carga_id
		...
		AND (
			unicTmp.unic_fecha_fin IS NULL OR unicTmp.unic_fecha_fin > equi_fecha_modificacion
		)
WHERE
	MABL_ES_ACTIVO = 1
