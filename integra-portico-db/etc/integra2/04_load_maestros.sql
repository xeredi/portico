...
WHERE NOT EXISTS (
    SELECT 1 FROM M_PUERTO_LOCAL
    WHERE codpue = subp_codigo
)
;
...
/* --------------------------- m_buque ------------------------------- */
/* --------------------------- m_buque ------------------------------- */
INSERT INTO M_BUQUE (CODBUQUE, CODIMO, FECALTA, FECMODIF, USRALTA, USRMODIF, IDAPLICANTERIOR, INDPDTEREV)
SELECT 
...
insert into m_buqueversion (
    ...
    , FECALTA, FECMODIF, USRALTA, USRMODIF, INDCERTMATRICULA, CODLLOYDS, CODTIPOPRESTACION, CODPUEBASE, IDNAVIERA, INDAPLIC195, INDTERCEROSPAISES, INDNOOPEAIS, INDGNL, INDIBIZASAV, NOMBRE)
SELECT * FROM (
    SELECT 
        (select trid_new_id from tbl_traduccion_ids_trid where trid_table_name = 'IESC_BUQUE_BUQU' AND trid_old_id = buqu_buque_id) as CODBUQUE
