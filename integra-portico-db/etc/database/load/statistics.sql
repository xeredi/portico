-- RECOPILACION DE ESTADISTICAS
EXEC DBMS_STATS.GATHER_SCHEMA_STATS('PORTICO');
EXEC DBMS_STATS.GATHER_SCHEMA_STATS('PORTICOADM');

EXEC DBMS_STATS.GATHER_TABLE_STATS('PORTICOADM','TBL_SERVICIO_SRVC');

ANALYZE TABLE TBL_subservicio_dato_ssdt COMPUTE STATISTICS;
...
from (
    select 
        a.table_name, a.constraint_name, listagg(a.column_name, ',') within
...
,(
    select table_name, index_name, listagg(c.column_name, ',') within
