			...
			if (tpprVO.getEntiHijasList() != null) {
				for (final Long entiId : tpprVO.getEntiHijasList()) {
					entiHijasMap.put(entiId,
							TipoSubparametroProxy.select(entiId));
				}
			}
			...
			for (final ParametroVO prmtVO : prmtList) {
				...
				if (tpprVO.getEntiHijasList() != null) {
					for (final Long entiId : tpprVO.getEntiHijasList()) {
						...
						sprmCriterioVO.setFechaVigencia(prmtCriterioVO
								.getFechaVigencia());
				...
				if (tpprVO.getI18n()) {
					i18nMap.putAll(I18nBO.selectMap(I18nPrefix.prvr, prmtVO
							.getPrvr().getId()));
				}

				try (final OutputStream stream = new FileOutputStream(
						"/temp/prmt_" + tpprVO.getId() + "_" + prmtVO.getId()
								+ ".pdf");) {
