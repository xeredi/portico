...
public final class ServicioPdfTest {
	...
	public void test() throws IOException, DRException {
		...
		final ServicioPdf srvcPdf = new ServicioPdf(new Locale(language,
				country));
		final Servicio srvc = BOFactory.getInjector().getInstance(
				Servicio.class);
		...
		for (final Long tpsrId : tpsrIds) {
			...
			if (tpsrVO.getEntiHijasList() != null) {
				for (final Long entiId : tpsrVO.getEntiHijasList()) {
					entiHijasMap.put(entiId,
							TipoSubservicioProxy.select(entiId));
				}
			}
			...
			for (final ServicioVO srvcVO : srvcList) {
				...
				if (tpsrVO.getEntiHijasList() != null) {
					for (final Long entiId : tpsrVO.getEntiHijasList()) {
						final Subservicio ssrv = BOFactory.getInjector()
								.getInstance(Subservicio.class);
						final SubservicioCriterioVO ssrvCriterioVO = new SubservicioCriterioVO();

						ssrvCriterioVO.setFechaVigencia(srvcCriterioVO
								.getFechaVigencia());
				...
				try (final OutputStream stream = new FileOutputStream(
						"/temp/srvc_" + tpsrVO.getId() + "_" + srvcVO.getId()
						+ ".pdf");) {
					srvcPdf.imprimir(srvcVO, tpsrVO, entiHijasMap, ssrvMap,
							stream);
				}
			}
		}
