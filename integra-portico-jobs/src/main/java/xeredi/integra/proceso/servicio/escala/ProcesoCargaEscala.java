...
public final class ProcesoCargaEscala extends ProcesoTemplate {
    ...
    /** The escala vo. */
    private final ServicioVO escalaVO = ServicioVO.newInstance(TipoServicioProxy.select(Entidad.ESCALA.getId()));

    /** The buque vo. */
    private final ParametroVO buqueVO = ParametroVO.newInstance(TipoParametroProxy.select(Entidad.BUQUE.getId()));
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (line.startsWith(BermanSegmento.BGM.name())) {
                ...
                if (fechaSolicitud != null) {
                    ...
                    escalaVO.setFreferencia(fechaSolicitud);
                }
    ...
    private void lecturaEscalaBerman(final List<String> lines, final int primeraLinea) {
        // Carga de la Escala
        SubservicioVO atraqueVO = SubservicioVO.newInstance(TipoSubservicioProxy.select(Entidad.ATRAQUE.getId()));
        SubservicioVO operacionVO = SubservicioVO.newInstance(TipoSubservicioProxy.select(Entidad.OPERACION_ATRAQUE
                .getId()));
        ...
        for (int i = primeraLinea; i < lines.size(); i++) {
            ...
            case FT1:
                ...
                case ZOB:
                    ...
                    break;
                case ZCL:
                    escalaVO.getItdtMap().get(TipoDato.COND_TANQUE_CARGA.getId())
                    .setCadena(BermanConstante.L.name());

                    break;
                case ZCV:
                    escalaVO.getItdtMap().get(TipoDato.COND_TANQUE_CARGA.getId())
                    .setCadena(BermanConstante.V.name());

                    break;
                case ZCI:
                    escalaVO.getItdtMap().get(TipoDato.COND_TANQUE_CARGA.getId())
                    .setCadena(BermanConstante.I.name());

                    break;
                case ZLL:
                    escalaVO.getItdtMap().get(TipoDato.COND_TANQUE_LASTRE.getId())
                    .setCadena(BermanConstante.L.name());
            ...
            case TSR:
                ...
                atraqueVO = SubservicioVO.newInstance(TipoSubservicioProxy.select(Entidad.ATRAQUE.getId()));
            ...
            case FT2:
                ...
                if (calificador == BermanConstante.ACB) {
                    ...
                    if (calificadorACB == BermanConstante.ZEP) {
                        ...
                        atraqueVO.getItdtMap().get(TipoDato.TIPO_ESTAN_ATR.getId())
                        .setCadena(BermanConstante.L.name());
                    }
                ...
                break;
            case HAN:
                operacionVO = SubservicioVO.newInstance(TipoSubservicioProxy.select(Entidad.OPERACION_ATRAQUE.getId()));
