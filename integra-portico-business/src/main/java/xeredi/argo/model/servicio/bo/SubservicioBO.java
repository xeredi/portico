...
public class SubservicioBO {
	 ...
	 * @throws InstanceNotFoundException
	 *             the instance not found exception
	 * @see #updatePostOperations(SqlSession, SubservicioVO)
	 */
	 ...
	 *            the ssrv vo
	 *
	 * @see #duplicatePostOperations(SqlSession, SubservicioVO)
	 */
	...
	public final void statechange(@NonNull final ItemTramiteVO ittr) throws ModelException {
		...
		try (final SqlSession session = SqlMapperLocator.getSqlSessionFactory().openSession(ExecutorType.BATCH)) {
			...
			// Modificacion de los datos del subservicio introducidos en el
			// tramite (si los hay)
			final SubservicioDatoDAO ssdtDAO = session.getMapper(SubservicioDatoDAO.class);
