...
public final class FacturaPdf extends BasePdf {
		...
		try {
			...
			for (final ValoracionLineaVO vlrl : vo.getVlrlList()) {
				if (vlrl.getRgla().getTipo() == ReglaTipo.T) {
					if (vlrlPrec != null) {
						// builder.add(createInfoLineasComponent(vlrlPrec,
						// vlrlMods));
					}
	...
	private ComponentBuilder<?, ?> createInfoLineasComponent(@NonNull final ValoracionLineaVO vlrl,
		...
		// columns.add("importe");
		//
		// report.addColumn(DynamicReports.col.column("conceptoTributario",
		// "conceptoTributario",
		// DynamicReports.type.stringType()).setWidth(6));
		// report.addColumn(DynamicReports.col.column("importeBase",
		// "importeBase",
		// DynamicReports.type.stringType())
		// .setWidth(2));
		// report.addColumn(DynamicReports.col.column("valorRegla",
		// "valorRegla",
		// DynamicReports.type.stringType())
		// .setWidth(2));
		// report.addColumn(DynamicReports.col.column("importe", "importe",
		// DynamicReports.type.stringType())
		// .setWidth(2));
		//
		// final DRDataSource dataSource = new DRDataSource(columns.toArray(new
		// String[] {}));
		//
		// for (final FacturaLineaVO fctlMod : fctlMods) {
		// linea.add(createEtiquetaValorComponent("Concepto Tributario",
		// fctlMod.getRgla().getCodigo()));
		//
		...
		// PdfConstants.DATE_FORMAT.format(itemVO
		// .getSpvr().getFinicio());
		...
		// objects[i++] = getItdtValue(entiVO.getEntdMap().get(tpdtId),
		// itemVO.getItdtMap().get(tpdtId));
		// }
		// }
		//
