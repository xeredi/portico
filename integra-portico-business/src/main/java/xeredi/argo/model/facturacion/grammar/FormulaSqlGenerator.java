...
public final class FormulaSqlGenerator extends FormulaBaseVisitor {
	...
	public String visitAritmethicExpr(final AritmethicExprContext ctx) {
		...
		if (ctx.lp != null) {
			return ctx.lp.getText() + visitAritmethicExpr(ctx.ae1)
					+ ctx.rp.getText();
		}

		if (ctx.fn != null) {
			if ("COALESCE".equals(ctx.fn.getText())) {
				return " COALESCE(" + visitAritmethicExpr(ctx.ae1) + ", "
						+ visitAritmethicExpr(ctx.ae2) + ")";
			}
		...
		if (ctx.opArit2 != null) {
			return visitAritmethicExpr(ctx.ae1) + ' ' + ctx.opArit2.getText()
					+ ' ' + visitAritmethicExpr(ctx.ae2);
		}
	...
	public String visitPath(final PathContext ctx) {
		...
		final AbstractEntidadDetailVO entiDetailBase = EntidadProxy
		...
		while (parseTreeIterator.hasNext()) {
			...
			if (parseTree instanceof PathElementContext) {
				...
				if (pathElementCtx.service != null) {
					...
					entiDetailElem = TipoServicioProxy.select(tpssDetail
							.getEnti().getTpsrId());
				...
				if (pathElementCtx.data != null) {
					...
					for (final Long tpdtId : entiDetailElem.getEntdList()) {
						final EntidadTipoDatoVO vo = entiDetailElem
					...
					case T:
						sqlElement += " tbl_servicio_dato_srdt WHERE srdt_tpdt_pk = "
								+ entd.getTpdt().getId()
								+ " AND srdt_srvc_pk = ";
						sqlElement += isFirst ? entiDetailBase.getEnti()
