...
public final class OppeFileImport {
	 ...
	 *            the lines
	 */
	public void readEAP(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines) {
		int i = 0;
	 ...
	 *            the lines
	 */
	public void readEAV(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines) {
		int i = 0;
	 ...
	 *            the lines
	 */
	public void readEAE(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines) {
		int i = 0;
	 ...
	 *            the lines
	 */
	public void readEMM(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines) {
		int i = 0;
		...
		for (final String line : lines) {
			...
			if (!line.isEmpty() && !line.startsWith(SIGMA_TOKEN)) {
				...
				estd.addItdt(TipoDato.TIPO_OP_BL.getId(), getTokenCR(EstadisticaFileKeyword.EMM_TipoOperacion, line, i,
						tpesDetail.getEntdMap().get(TipoDato.TIPO_OP_BL.getId()).getTpdt()));
				...
				estd.addItdt(TipoDato.TIPO_TRANSPORTE.getId(), getTokenCR(EstadisticaFileKeyword.EMM_TipoTransporte,
						line, i, tpesDetail.getEntdMap().get(TipoDato.TIPO_TRANSPORTE.getId()).getTpdt()));
				estd.addItdt(TipoDato.DECIMAL_01.getId(),
	 ...
	 *            the is sigma
	 */
	public void readEME(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines, final boolean isSigma) {
		int i = 0;
	 ...
	 *            the lines
	 */
	public void readEMT(final TipoEstadisticaDetailVO tpesDetail, final List<String> lines) {
		int i = 0;
	...
	private String getTokenCR(final EstadisticaFileKeyword keyword, final String line, final int lineNumber,
		...
		// final TipoDatoVO tpdt = TipoDatoProxy.select(tipoDato.getId());

		if (!tpdt.getCdrfCodeSet().contains(codigo)) {
			proceso.addError(MensajeCodigo.G_004, "archivo: " + keyword.getFileType().name() + ", linea: " + lineNumber
					+ ", CR: " + tpdt.getCodigo() + ", codigo: " + codigo);
		}
